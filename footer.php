				...
				<div class="container">
					<div class="row-fluid">
						<div class="col-md-12">
							...
							<div id="credit">
								<?php
                        $bcd_url = 'http://brooklyncreative.com';
                        $theme_url = 'http://brooklyncreative.com/lean';
                        ?>
                        ...
                        <?php if( null != get_page_by_path( 'privacy-policy' ) && 0 != get_page_by_path( 'privacy-policy' )->ID && 'publish' == get_page_by_path( 'privacy-policy' )->post_status ) { ?>
                            <?php printf( __( '&copy; %1$s %2$s &mdash; %3$s &mdash; %4$s by %5$s', TRANSLATION_KEY ), date( 'Y' ), '<a href="' . home_url() . '">' . get_bloginfo( 'name' ) . '</a>', '<a href="' . get_permalink( get_page_by_path( 'privacy-policy' )->ID ) . '">Privacy Policy</a>', '<a href="' . $theme_url . '" target="_blank">' . THEME_NAME . '</a>', '<a href="' . $bcd_url . '" target="_blank">BCD</a>' ); ?>
                        <?php } else { ?>
                            <?php printf( __( '&copy; %1$s %2$s &mdash; %3$s by %4$s', TRANSLATION_KEY ), date( 'Y' ), '<a href="' . home_url() . '">' . get_bloginfo( 'name' ) . '</a>', '<a href="' . $theme_url . '" target="_blank">' . THEME_NAME . '</a>', '<a href="' . $bcd_url . '" target="_blank">BCD</a>' ); ?>
                        <?php } // end if/else ?>
